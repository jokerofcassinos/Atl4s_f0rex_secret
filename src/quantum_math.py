...
class QuantumMath:
    ...
    @staticmethod
    def calculate_hurst_exponent(series: pd.Series, window=100, last_only=True):
        """
        Estimates Hurst Exponent. 
        last_only: If True, only calculates for the last window to save time.
        """
        ...
        if last_only:
            return _robust_hurst(series.iloc[-window:].values if len(series) >= window else series.values)
        return series.rolling(window=window).apply(_robust_hurst)

    @staticmethod
    def fisher_information_curvature(series: pd.Series, window=20, last_only=True):
        """
        Calculates the Scalar Curvature of the Statistical Manifold.
        """
        def _fisher_r(data):
        ...
        if last_only:
            return _fisher_r(series.iloc[-window:].values if len(series) >= window else series.values)
