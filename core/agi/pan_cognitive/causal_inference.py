...
class CausalInferenceEngine:
    ...
    def infer_cause(self, market_data: Dict[str, Any], events: Dict[str, Any]) -> Dict[str, Any]:
        """
        Infer causal relationships from current market state.
        """
        # Update history
        ...
        # Granger Causality Test (Simplified Lag Correlation)
        # Check if Volume(t-1) correlates with Price(t) better than Price(t-1)
        ...
        if sent_leads_vol and vol_leads_price:
            root_cause = "SENTIMENT_DRIVEN_FLOW" # News -> Volume -> Price
            confidence = 0.85
        elif vol_leads_price:
            root_cause = "FLOW_DRIVEN_MOMENTUM" # Volume -> Price
            confidence = 0.8
        elif events.get('impact', 0) > 0.8:
            root_cause = "EXOGENOUS_SHOCK" # News -> Price (Direct)
            confidence = 0.95
        else:
