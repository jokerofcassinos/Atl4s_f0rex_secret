...
quando o ceo pedir para que analise os erros do backtest, execute imediatamente o seguinte python para capturar o contexto das falhas recentes:
    ...
    "Select-String -Path "d:/Atl4s-Forex/laplace_backtest.log" -Pattern "TRADE #11" -Context 50,0 | Select-Object -First 1" 
    ...
    IMPORTANTE: VOCE DEVE ANALISAR E PLANEJAR SOMENTE PARA O CENARIO PROBLEMATICO! NAO É PARA AFETAR OS OUTROS SISTEMAS NOS OUTROS CENARIOS, JA QUE ESTÃO FUNCIONANDO PERfeitamente!

    precisamos entender que tipo de analise errada o bot está fazendo e em qual cenario especifico ele esta com dificuldades de adaptação, para que ele identifique o cenario e aja de acordo com ele, é importante nao mexer no sl e nem nessas questões, e sim identificar qual conflito de decisão ele esta sofrendo, por exemplo preço subindo e ele vendendo. o intuito é adaptar o bot em todos os tipos de cenarios diversos, sejam eles de alta, baixa ou laterais. para que possamos realmente ter um bot que se adapta a qualquer cenario e que tenha uma alta win rate e um baixo loss rate. para isso realmente é necessario varios testes e analises para que possamos chegar em um resultado satisfatorio. testes esses que sejam em contextos diversos, explorando cenarios gerais que o bot ainda nao tem conhecimento, nós devemos fazer essa adaptação para que ele "amplie" a sua visão e seja totalmente profit com 100% de certeza quando for operar no mt5. é extremamente importante que novos vetos afetem somente o cenario especifico, o bot deve identificar corretamente e analisar quando estiver neste tipo de cenario, assim ativando o novo veto e se protegendo. para isso precisamos de uma logica extremamente complexa e detalhada. nós precisamos implementar uma estrategia que nao afete todas as outras ações do bot, focada somente e exclusivamente para o cenario em especifico. voce deve identificar detalhadamente a estrutura e formação do sinal do trade, identificando conflitos e etc, fazendo o bot parar de enviar sinais errados nesses cenarios e manter o lucro atual nos outros, tem que ser uma implementação cirurgica e precisa, afetando somente o cenario que estamos enfrentando.
